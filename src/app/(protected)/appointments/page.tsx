...
export default function AppointmentsPage() {
  ...
  const [withMeAppointments, setWithMeAppointments] = useState<Appointment[]>(
    []
  );
    ...
    const fetchAppointments = async () => {
      ...
      const formatBookings = async (bookings: any[]) => {
        ...
        return await Promise.all(
          bookings.map(async (item: any, idx: number) => {
            ...
            if (item.TaskID) {
              try {
                const taskRes = await fetch(
                  `${API_BASE_URL}/api/tasks/get/${item.TaskID}`,
                  {
                    headers: { Authorization: `Bearer ${token}` },
                  }
                );
                if (taskRes.ok) {
                  ...
                  taskOwnerEmail =
                    taskData.Author?.Email || taskData.Author?.email;
                  if (!dateStr || !from || !to) {
  ...
  const handleSendFirstMessage = async (appt: Appointment) => {
    if (!appt || !appt.taskId || !appt.taskOwnerId) {
      setDialog({
        open: true,
        message: "No task or owner info found.",
        isError: true,
      });
      return;
    }
                      ...
                      {appt.status === "upcoming" && (
                        <div className="flex justify-end mt-2">
                          <button
                            ...
                            onClick={() =>
                              setMessageModal({ open: true, appt })
                            }
                          >
                            💬 Message
                          </button>
